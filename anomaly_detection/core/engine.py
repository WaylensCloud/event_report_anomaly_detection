...
class AnomalyDetectorEngine:
    ...
    def _candidate_segment_is_coherent(
        ...
        if len(y) == 2:
            return abs(float(y.iloc[-1] - y.iloc[0])) <= max(level * tolerance_ratio, min_deviation * 0.25, 1.0)
        ...
        tolerance = max(level * tolerance_ratio, step_median * 1.5, min_deviation * 0.25, 1.0)
    ...
    def _run_periodic_phase_anomaly(
        ...
        """
        强制周期相位异常检测：
        对每个目标日 D，只使用 D 之前同相位历史点进行预测和阈值计算。
        例如 period=7 时，第 D 天只和 D-7、D-14、D-21... 比较。
        ...
        phase_states = {
            phase: {
                ...
                "last_warmup_deviation": 0.0,
                "post_warmup_grace_remaining": 0,
                "post_warmup_grace_used": False
            }
        ...
        for i in range(len(raw_y)):
            ...
            if len(confirmed_history) >= min_phase_history:
                if (
                    len(confirmed_history) == min_phase_history
                    and state["last_warmup_deviation"] > 0
                    and not state["post_warmup_grace_used"]
                ):
                    state["post_warmup_grace_remaining"] = max(state["post_warmup_grace_remaining"], 1)
                    state["post_warmup_grace_used"] = True
                dynamic_min = max(min_deviation, abs(expected) * expected_relative_tolerance)
                raw_width = scale * threshold_multiplier
                ...
                threshold_width = max(min_deviation, abs(expected) * expected_relative_tolerance)
                eligible = False
            ...
            normal_dispersion_width = AnomalyDetectorEngine._normal_dispersion_width(
                confirmed_history,
                expected,
                min_deviation,
                normal_dispersion_multiplier,
                normal_dispersion_floor_ratio,
                normal_dispersion_min_points
            )
            # threshold_width = max(threshold_width, normal_dispersion_width)
            # lower_threshold_width = max(
            #     lower_threshold_width,
            #     normal_dispersion_width * lower_anomaly_tolerance_multiplier
            # )
            ...
            eligible_for_anomaly.append(eligible)

            deviation = actual - expected
            abs_deviation = abs(deviation)
            dynamic_scale_value = max(threshold_width / max(threshold_multiplier, 1e-6), 1.0)
            score_scale_value = dynamic_scale_value
            if deviation >= 0:
                is_point_anomaly = bool(eligible and deviation > threshold_width)
            else:
                is_point_anomaly = bool(eligible and abs_deviation > lower_threshold_width)

            strong_regime_break = False
            if not is_point_anomaly and len(confirmed_history) >= 2:
                ...
                local_stability_width = max(
                    min_deviation,
                    abs(local_level) * stable_regime_tolerance_ratio,
                    local_step * 2.5,
                    1e-9
                )
                is_recent_history_stable = local_spread <= max(local_stability_width * 1.5, min_deviation)
                shock_width = local_stability_width if is_recent_history_stable else max(
                    min_deviation,
                    threshold_width,
                    abs(local_level) * expected_relative_tolerance,
                    local_step * 2.5,
                    1e-9
                )
                upper_reference = max(expected, local_level)
                lower_reference = min(expected, local_level)
                high_shift_is_material = (
                    actual >= upper_reference * (1.0 + stable_shift_min_ratio)
                    or actual - upper_reference >= min_deviation * 2.0
                )
                low_shift_is_material = (
                    actual <= lower_reference * max(0.0, 1.0 - stable_shift_min_ratio)
                    or lower_reference - actual >= min_deviation * 2.0
                )
                if actual > upper_reference + shock_width and high_shift_is_material:
                    is_point_anomaly = True
                    strong_regime_break = True
                    score_scale_value = max(shock_width / max(threshold_multiplier, 1e-6), 1.0)
                elif actual < lower_reference - shock_width and low_shift_is_material:
                    is_point_anomaly = True
                    strong_regime_break = True
                    score_scale_value = max(shock_width / max(threshold_multiplier, 1e-6), 1.0)

            if eligible and state["post_warmup_grace_remaining"] > 0 and not strong_regime_break:
                warmup_width = max(
                    state["last_warmup_deviation"] * 1.25,
                    threshold_width,
                    lower_threshold_width if deviation < 0 else threshold_width
                )
                if abs_deviation <= warmup_width:
                    is_point_anomaly = False
                state["post_warmup_grace_remaining"] -= 1
            elif not eligible:
                state["last_warmup_deviation"] = max(state["last_warmup_deviation"], abs_deviation)
        ...
        expected_curve.attrs['model_name'] = 'periodic_phase_regime_variance_robust'
